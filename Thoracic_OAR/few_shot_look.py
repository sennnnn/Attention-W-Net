...
plt.title('No-process dice:%.3f'%(np_dice_index(one_hot(raw[index],7),one_hot(real[index],7))))
plt.axis('off')
plt.subplot(133)
plt.imshow(grayToRgb(process[index]),cmap='gray')
plt.title('After-process dice:%.3f'%(np_dice_index(one_hot(raw[index],7),one_hot(process[index],7))))
plt.axis('off')
plt.show()